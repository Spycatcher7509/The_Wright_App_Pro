    ...
    if (isActive && canvasRef.current && analyserRef.current) {
      ...
      const draw = () => {
        ...
        for (let i = 0; i < bufferLength; i++) {
          ...
          ctx.fillStyle = `rgb(${99 + (i * 2)}, 102, 241)`;
  ...
  const startSession = async () => {
    try {
      ...
      if (!key || key === "undefined") {
        throw new Error("Security Protocol Alert: No API Key found. Ensure the 'API_KEY' is provisioned in the site environment.");
      }
        ...
        callbacks: {
          ...
          onmessage: async (msg: LiveServerMessage) => {
            ...
            if (msg.serverContent?.turnComplete && currentTurn) {
              setTranscriptions(prev => [...prev, { ...currentTurn!, time: new Date().toLocaleTimeString('en-GB', { hour12: false }) }]);
              setCurrentTurn(null);
            }
        ...
        <div className="w-96 flex flex-col gap-6">
          <div className="bg-slate-900 rounded-[2.5rem] p-10 border border-slate-800 shadow-2xl flex flex-col items-center justify-center text-center relative overflow-hidden h-fit">
            <div className="relative z-10 space-y-8 w-full">
              <div className="relative w-48 h-48 mx-auto flex items-center justify-center">
                <canvas ref={canvasRef} width={200} height={200} className={`absolute inset-0 rounded-full transition-opacity duration-1000 ${isActive ? 'opacity-100' : 'opacity-0'}`} />
                <div className={`relative z-10 w-32 h-32 rounded-full border-4 flex items-center justify-center text-5xl transition-all duration-500 ${isActive ? 'border-indigo-500 bg-indigo-500/10' : 'border-slate-800 bg-slate-800/50'}`}>
                  {isActive ? '🎙️' : '💤'}
                </div>
              </div>
              <div>
              ...
              <button onClick={isActive ? stopSession : startSession} className={`w-full py-5 rounded-2xl font-black uppercase tracking-widest text-xs transition-all ${isActive ? 'bg-rose-600 text-white hover:bg-rose-700' : 'bg-indigo-600 text-white hover:bg-indigo-700 shadow-xl shadow-indigo-900/40'}`}>
          ...
          <div className="bg-white rounded-[2.5rem] p-8 border border-slate-200 shadow-xl flex-1 flex flex-col">
            <div className="flex justify-between items-center mb-6 border-b border-slate-100 pb-4">
              ...
              <button onClick={downloadSessionLog} disabled={transcriptions.length === 0} className="text-indigo-600 hover:text-indigo-800 text-[9px] font-black uppercase disabled:opacity-0">Archive Log</button>
            </div>
            <div className="space-y-6">
              <div className="space-y-2">
                <div className="flex justify-between text-[9px] font-bold uppercase"><span className="text-slate-400">Signal Integrity</span><span className={isActive ? "text-indigo-600" : "text-slate-300"}>{isActive ? '99.9%' : '0%'}</span></div>
                <div className="h-1 bg-slate-100 rounded-full overflow-hidden"><div className={`h-full bg-indigo-500 transition-all duration-500 ${isActive ? 'w-full' : 'w-0'}`}></div></div>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="flex-1 overflow-y-auto p-10 space-y-6 custom-scrollbar z-10">
            {transcriptions.map((t, i) => (
              <div key={i} className={`flex flex-col ${t.role === 'user' ? 'items-end' : 'items-start'}`}>
                <div className={`p-6 rounded-[2rem] text-sm leading-relaxed max-w-[80%] ${t.role === 'user' ? 'bg-indigo-600 text-white rounded-tr-none' : 'bg-slate-800 text-slate-200 border border-slate-700 rounded-tl-none'}`}>{t.text}</div>
                <div className="mt-2 px-4 flex items-center gap-2"><span className="text-[8px] font-black text-slate-600 uppercase tracking-widest">{t.role === 'user' ? user.name : 'Wright Engine'}</span><span className="text-[8px] font-mono text-slate-700">{t.time}</span></div>
              </div>
            ))}
            {currentTurn && (
              <div className={`flex flex-col ${currentTurn.role === 'user' ? 'items-end' : 'items-start'} opacity-60 animate-pulse`}>
                <div className={`p-6 rounded-[2rem] text-sm leading-relaxed max-w-[80%] ${currentTurn.role === 'user' ? 'bg-indigo-600 text-white rounded-tr-none' : 'bg-slate-800 text-slate-200 border border-slate-700 rounded-tl-none'}`}>{currentTurn.text}</div>
              </div>
            )}
